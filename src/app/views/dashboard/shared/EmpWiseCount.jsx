...
const EmpWiseCount = () => {
    ...
    const [EmpWiseCount, setEmpWiseCount] = useState([]);
    ...
    const getDashboardDataFetch = () => {
        ...
        axios.get(`http://43.204.38.243:3001/api/getRolewiseClosedCount`, {
            ...
            .then((response) => {
                setEmpWiseCount(response.data.data);
    ...
    return (
        <Grid container spacing={4} sx={{ mb: '24px' }}>
            {EmpWiseCount.map((item, index) => (
                <Grid item xs={12} md={3} key={index}>
                    <StyledCard elevation={11} style={{ boxShadow: '80px 90px' }} >
                        <ContentBox>

                            <Icon style={{ fontSize: "65px", color: '#7499e5' }} >person</Icon>

                            <Box ml="12px">
                                <Small fontSize={14} >{item.empName}</Small>
